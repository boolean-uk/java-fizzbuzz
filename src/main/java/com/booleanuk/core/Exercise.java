...
public class Exercise {
    ...
    public String[] fizzbuzz(String[] nums) throws NumberFormatException {
        ...
        * */
        String[] fizzedAndBuzzed = new String[nums.length];
        for(int i = 0; i < nums.length; i++) {
            int asInteger = Integer.parseInt(nums[i]);
            if (asInteger % 3 == 0 && asInteger % 5 == 0) {
                fizzedAndBuzzed[i] = "Fizzbuzz";
            } else if (asInteger % 3 == 0) {
                fizzedAndBuzzed[i] = ("Fizz");
            } else if (asInteger % 5 == 0) {
                fizzedAndBuzzed[i] = ("Buzz");
            } else {
                fizzedAndBuzzed[i] = nums[i];
            }
        }
